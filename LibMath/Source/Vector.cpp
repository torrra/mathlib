...
namespace LibMath
{
		...
		Vector3			Vector3::cross(Vector3 const& other) const
		{
			float		prodX = (m_y * other.m_z) - (m_z * other.m_y),
						prodY = (m_x * other.m_z) - (m_z * other.m_x),
	...
	//std::ostream& operator<<(std::ostream& os, Vector3 const&)
	//{
	//	//os << rhs.string();
	...
	//std::istream& operator>>(std::istream& is, Vector3&)
	//{
	//	return is;
